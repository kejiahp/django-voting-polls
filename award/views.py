...
def search_category(request):
    if request.method == 'POST':
        ...
        categories = AwardsCategory.objects.filter(name__icontains = search_result)
        ...
        if categories:
            ...
            context = {"message":f"Sorry couldn't find results for search: {search_result}"}
            return render(request,"category_search.html",context)
...
def verify_vote(request,ref):
    ...
    if response["status"]==True and response["message"]=='Verification successful' and response['data']['status'] == 'success' and response['data']['amount'] == voter_paid:
        ...
        cont_voted = get_object_or_404(AwardsContestant, id = voter.contestant_id.id)
